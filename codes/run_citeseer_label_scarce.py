...
opt['dataset'] =  '../data/graph_datasets/citeseer' # '../data/citeseer'
opt['hidden_dim'] = 16
...
label_rate = float(sys.argv[1]) / 10
opt['split_rate'] = label_rate
opt['log'] = 'citeseer_label_scarce_{}.txt'.format(label_rate)

if os.path.exists(opt['log']):
    os.system('rm %s' %opt['log'])

def generate_command(opt):
    cmd = 'python3 train_label_scarce.py'
