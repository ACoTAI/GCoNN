...
# randomly drop some node's attr
drop_rate = opt['drop_rate']
drop_idx_test = np.random.choice(range(len(idx_test)), size=int(drop_rate*len(idx_test)), replace=False)
drop_idx_dev = np.random.choice(range(len(idx_dev)), size=int(drop_rate*len(idx_dev)), replace=False)
drop_node_index = np.concatenate([idx_test[drop_idx_test], idx_dev[drop_idx_dev]])
# dropout node feature
inputs_pa[drop_node_index] = torch.zeros(opt['num_feature'])
inputs_q[drop_node_index] = torch.zeros(opt['num_feature'])

# evaulating the performance on dropout node
if "dropout_accuracy" in opt['log']:
    idx_test = idx_test[drop_idx_test]
