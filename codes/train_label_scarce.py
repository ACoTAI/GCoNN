...
parser.add_argument('--split_rate', type=float, default=0., help='label scarce rate')
...
# spliting trianing data by the provided label ratio
train_mask = np.zeros(opt['num_node'], dtype=np.bool)
num_nodes = int(opt['num_node']*opt['split_rate'])
train_idx = np.random.choice(opt['num_node'], num_nodes, replace=False)
train_mask[train_idx] = True
val_mask = np.bitwise_not(train_mask)
idx_train = idx_all[train_mask]
idx_dev = idx_test = idx_all[val_mask]
